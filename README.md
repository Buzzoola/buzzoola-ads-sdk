...
Логи успешной интеграции отображаются в консоли с указанием [Ads SDK]. В случае ошибки при интеграции - в логах появится сообщение.
...
Если вы вызовете loadAd, не вызвав предварительный destroy, или же когда выполнение предыдущего loadAd не было успешно завершено - вы получите ошибку в лог.
...
#### Проекты на SwiftUI
Для реализации баннерной рекламы на проектах, использующих SwiftUI, необходимо делать обертку UIViewRepresentable.
Пример: 
...
#### Проекты на SwiftUI
Для реализации нативной рекламы на проектах, использующих SwiftUI, необходимо делать обертку UIViewRepresentable с реализацией вашей кастомной вью на UIKit.
Пример: 
...
struct NativeUIKitView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        context.coordinator.adLoader?.loadAd(request: AdsRequest(placementID: placementID))
    ...
    class Coordinator: NSObject, NativeAdLoaderProtocol, NativeAdDelegate, ImageLoadingDelegate {
        ...
        func onAdsLoaded(ads: [any BuzzoolaAdsSDK.NativeAd]) {
            ...
            let ad = ads.first
        ...
        func onAdsFailed(adError: BuzzoolaAdsSDK.AdError) {
...
#### Проекты на SwiftUI
Для реализации полноэкранной рекламы на проектах, использующих SwiftUI, необходимо делать обертку UIViewControllerRepresentable.
Пример: 
...
#### Проекты на SwiftUI
Для реализации полноэкранной рекламы с вознаграждением на проектах, использующих SwiftUI, необходимо делать обертку UIViewControllerRepresentable.
Пример: 

```ruby

struct RewardedView: UIViewControllerRepresentable {

    func makeUIViewController(context: Context) -> UIViewController {
        let controller = UIViewController()
        context.coordinator.viewController = controller
        context.coordinator.adLoader = RewardedAdLoader()
        context.coordinator.adLoader?.delegate = context.coordinator
